    ...
    <div className="bg-white rounded-lg shadow-md p-6 mb-8">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-end">
        ...
        <div className="w-full md:w-1/3">
          ...
          <select 
            className="w-full border border-gray-300 rounded-md p-2 focus:ring-2 focus:ring-blue-500" value={selectedStatId}
            onChange={(e) => onSelectStatId(Number(e.target.value))}>

            {STATISTIEK_OPTIES.map(statistiek => (
              <option key={statistiek.id} value={statistiek.id}>{statistiek.label}</option>
            ))}
          </select>
        </div>
        ...
        <div className="w-full md:w-1/4">
          ...
          <input type="number" min="1999" max="2024" value={year}
            disabled={!currentOption?.needsYear} className={`w-full border border-gray-300 rounded-md p-2 focus:ring-2 focus:ring-blue-500 ${!currentOption?.needsYear ? 'bg-gray-100 text-gray-400' : ''}`}
